...
export class UserGroupRoleController {
  ...
  @Delete('del/:id')
  @ApiParam({ name :'id' , description:'用户组角色表的id' })
  ...
  @Get('role/:user_group_id')
  @ApiParam({ name :'user_group_id' , description:'用户组ID' })
  @ApiOperation({ summary: '用户组拥有的角色', description: '根据用户组id来查询这个用户组下所有的角色' }) 
  async findAll(@Param('user_group_id') user_group_id: string) {
    try {
