...
export class UserGroupService {
  ...
  async remove(id: string) {
    let session = await this.userGroupSchema.startSession(); 
    session.startTransaction();
    let result;
    try {
      // 查询所有组
      let userGroups = await this.userGroupSchema.find({},{}).lean()
      // 根据ID查询该组下所有成员
      let children = sonsTree(treeFormat(userGroups),id)
      // 返回一个只有id的数组，在加上要删除的ID
      let ids = getTreeIds(children).concat(id)
      // 删除该组和该组的子集
      result = await this.userGroupSchema.deleteMany({_id: {$in: ids}})
      // 循环改成将ids改成ObjectId
      let o_ids = [];
      ids.forEach((item) => {
      ...
      console.log(await this.userGroupRoleSchema.deleteMany({ user_group_id : {$in: o_ids } }),o_ids)
